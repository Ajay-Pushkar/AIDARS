...
- **M4-C Deduplication & Construction**: `PackagePlanner` performs content-addressed deduplication (identical SHA-256 hashes = single physical copy). `PackageBuilder` copies assets to canonical directory layout, writes deterministic schema v1.0 manifest (`sort_keys=True`, assets sorted by `asset_id`).
- **M4-D Validation**: `PackageValidator` re-hashes every file in the package and compares against the manifest. Flags tampered or missing files.
...
- **Security Hardening**: Path traversal defense (`is_relative_to`), symlink escape prevention (`src.resolve()`), duplicate basename collision avoidance (SHA-256 hash prefixes), atomic package creation (`.tmp` staging → `os.replace`).
